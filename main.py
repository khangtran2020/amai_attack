...
def run(args, device):
    ...
    train_loader = torch.utils.data.DataLoader(
        AMIADatasetCelebA(target, transform, args.data_path, True, imgroot=None, multiplier=2000), shuffle=False,
        num_workers=0, batch_size=200000)
    test_loader = torch.utils.data.DataLoader(
        AMIADatasetCelebA(target, transform, args.data_path, False, imgroot=None, multiplier=100), shuffle=False,
        num_workers=0, batch_size=200000)
    if args.mode == 'train':
