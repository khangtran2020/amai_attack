...
def train(args, device, data, model):
    ...
    if (args.train_mode == 'dp'):
        ...
        print("Train with Laplace mechanism with epsilon = {}".format(args.epsilon))
        temp_x = x_train.numpy()
        temp_x[10:args.train_multiplier] = temp_x[10:args.train_multiplier] + np.random.laplace(0, args.sens * args.num_feature / args.epsilon,
                                                    temp_x[10:args.train_multiplier].shape)
        x_train = torch.from_numpy(temp_x)
        temp_x = x_valid.numpy()
        temp_x[1:args.valid_multiplier] = temp_x[1:args.valid_multiplier] + np.random.laplace(0, args.sens * args.num_feature / args.epsilon,
