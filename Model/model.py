...
def train(args, target, device, data, model):
    ...
    for step in range(args.num_steps):
        ...
        loss = criteria(probs.cpu(), y_train.cpu()) + triplet_loss(fc1[:args.train_multiplier].cpu(), fc1[torch.randperm(args.train_multiplier)].cpu(), fc1[args.train_multiplier + torch.randperm(args.train_multiplier)].cpu())
        loss_value += loss
        predictions = fc2[:, 0] < 0
