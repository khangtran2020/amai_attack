...
def _userdp_mech_mult(delta):
    return math.sqrt(math.log(1 / delta))


def _userdp_robustness_size(p_max_lb, p_sec_ub, attack_size, dp_epsilon, dp_delta):
    if p_max_lb <= p_sec_ub:
        # we're not even robust to the measurement error...
        return 0.0
    ...
    #  epsilon_range = list(np.arange(0.1, 1.00000001, 0.001))  # we want 1 included
    for delta in delta_range:
        eps_min, eps_max, eps = (0.0, 10.0, 5.0)
        while eps_min < eps and eps_max >= eps:
            #  for eps in epsilon_range:
            l = attack_size * \
                (eps / dp_epsilon) * \
                (_userdp_mech_mult(dp_delta) / _userdp_mech_mult(delta))
            if p_max_lb >= math.e ** (2 * eps) * p_sec_ub + (1 + math.e ** eps) * delta:
                if l > max_r:
                    max_r = l
                    max_r_eps = eps
...
def robustness_size_argmax(counts, eta,
    ...
    elif dp_mechanism == 'userdp':
        return _userdp_robustness_size(p_max_lb, p_sec_ub, dp_attack_size, dp_epsilon, dp_delta)
    else:
        raise ValueError('Only supports the following DP mechanisms: laplace gaussian.')


def robustness_size_softmax(args, tot_sum, sqr_sum, counts, eta,
                            dp_attack_size, dp_epsilon, dp_delta, dp_mechanism):
